...
function getVisiblePrintedPhoneNumber() {
  ...
  const phoneNodes = Array.from(document.querySelectorAll<HTMLElement>("[data-ringba-phone]"));
...
export default function BenchCallPage({
  ...
  const ringbaTags = JSON.stringify({
    funnel_id: funnelId,
    call6_age_group: ageGroup,
    call6_insurance_goal: insuranceGoal,
    lead_id: leadId,
  });
  const basePhoneNumber = normalizePhoneDigits(phoneNumber);

  useEffect(() => {
    if (!leadId || hasSentPrintedNumberRef.current) return;

    const sendPrintedNumberIfReady = () => {
      if (hasSentPrintedNumberRef.current) return;

      const printedNumber = getVisiblePrintedPhoneNumber();
    ...
    const observer = new MutationObserver(sendPrintedNumberIfReady);
    ...
    const intervalId = window.setInterval(sendPrintedNumberIfReady, 500);
    const timeoutId = window.setTimeout(() => {
      window.clearInterval(intervalId);
      observer.disconnect();
    }, 15000);

    sendPrintedNumberIfReady();
  ...
  return (
    <main className="min-h-screen bg-white text-slate-800">
      <Script id="ringba-call-6-tags" strategy="afterInteractive">
        {`
      ...
      {ringbaScriptUrl ? (
        <Script id="ringba-call-6-number-pool" src={ringbaScriptUrl} strategy="afterInteractive" />
      ) : null}
      ...
      <header className="border-b border-slate-200 bg-white">
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
          ...
          <a data-ringba-phone href={telHref} onClick={trackContactClick} className="inline-flex items-center gap-2 whitespace-nowrap text-sm font-bold sm:text-base" style={{ color: HERO_BG }}>
      ...
      <section id="top" style={{ backgroundColor: HERO_BG }} className="text-white">
        <div className="mx-auto max-w-7xl px-5 pb-20 pt-10 text-center lg:px-8 lg:pb-28 lg:pt-14">
          ...
          <div className="mt-10">
            <a data-ringba-phone href={telHref} onClick={trackContactClick} className="block whitespace-nowrap text-[clamp(1.75rem,9vw,2.5rem)] font-bold tracking-tight md:text-3xl">
              {phoneDisplay}
            </a>
            <a
              href={telHref}
      ...
      <section className="bg-white px-5 py-20 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-12 text-center">
            <a data-ringba-phone href={telHref} onClick={trackContactClick} className="inline-flex items-center gap-3 rounded-md px-8 py-4 text-lg font-bold text-white shadow-lg" style={{ backgroundColor: ACCENT }}>
      ...
      <section className="bg-slate-50 px-5 py-20 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-12 flex flex-col items-center gap-4">
            <a data-ringba-phone href={telHref} onClick={trackContactClick} className="whitespace-nowrap text-[clamp(1.5rem,7vw,2.25rem)] font-extrabold md:text-4xl" style={{ color: HERO_BG }}>{phoneDisplay}</a>
      ...
      <section style={{ backgroundColor: HERO_BG }} className="px-5 py-20 text-white lg:px-8">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <div className="mt-10 flex flex-col items-center gap-4">
            <a data-ringba-phone href={telHref} onClick={trackContactClick} className="whitespace-nowrap text-[clamp(1.75rem,8vw,3rem)] font-extrabold md:text-5xl">{phoneDisplay}</a>
            <a href={telHref} onClick={trackContactClick} className="inline-flex items-center gap-3 rounded-md px-8 py-4 text-lg font-bold text-white shadow-lg" style={{ backgroundColor: ACCENT }}>
      ...
      <footer className="bg-slate-900 px-5 py-12 text-slate-300 lg:px-8">
        <div className="mx-auto max-w-7xl space-y-3 text-xs leading-relaxed">
          ...
          <p>Better Life is an independent promotional and advertising service. This website and the services offered are not sponsored, affiliated with, endorsed, or administered by Facebook. The content on this site has not been reviewed, approved, or certified by Facebook or any of its related entities.</p>
        </div>
      </footer>
    </main>
  );
