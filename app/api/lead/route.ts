...
function getLeadTokenValidationDetails(request: Request) {
  const headerToken = request.headers.get("x-lead-token")?.trim();
  const cookieToken = getRequestCookie(request, leadTokenCookieName).trim();
  ...
  return {
    ...
    matches: Boolean(headerToken && cookieToken && headerToken === cookieToken),
  };
}
...
export async function POST(request: Request) {
  ...
  if (!hasValidLeadToken(request)) {
    console.warn("Lead token validation skipped", getLeadTokenValidationDetails(request));
  }
